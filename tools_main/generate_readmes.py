...
def gather_files(paths: Iterable[str], files: Iterable[str], recurse: bool, *,
                 project_root: Path,
                 include_globs: Optional[List[str]] = None,
                 exclude_globs: Optional[List[str]] = None) -> List[Path]:
    """Descubre archivos .py respetando includes/excludes.

    - include_globs: patrones glob opcionales (relative to project_root) que deben cumplirse
    - exclude_globs: patrones glob opcionales para *excluir* (tienen prioridad)
    """
    include_globs = include_globs or []
    # Excludes por defecto para evitar "bombas":
    default_excludes = [
        ".git/**", "**/.git/**", "**/.venv/**", "venv/**", "env/**",
        "**/__pycache__/**", "**/.mypy_cache/**", "**/.pytest_cache/**",
        "**/node_modules/**", "**/.idea/**", "**/.vscode/**",
        "**/build/**", "**/dist/**", "**/site-packages/**",
        "docs/**",  # evita re-leer docs generados
    ]
    ...
    def is_excluded(p: Path) -> bool:
        ...
        s = str(rel)
        return any(rel.match(g) or Path(s).match(g) for g in exclude_globs)
    ...
    def is_included(p: Path) -> bool:
        ...
        rel = p.relative_to(project_root)
        s = str(rel)
        return any(rel.match(g) or Path(s).match(g) for g in include_globs)
    ...
    for f in files:
        ...
        if p.is_file() and p.suffix == ".py" and not is_excluded(p) and is_included(p):
            found.append(p)

    # Rutas/directorios
    for p_str in paths:
        p = (project_root / p_str).resolve()
        if p.is_file() and p.suffix == ".py":
            if not is_excluded(p) and is_included(p):
                found.append(p)
        elif p.is_dir():
            glob_pat = "**/*.py" if recurse else "*.py"
            for candidate in sorted(p.glob(glob_pat)):
                if candidate.is_file() and candidate.suffix == ".py":
                    if not is_excluded(candidate) and is_included(candidate):
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    # NUEVO: filtros
    p.add_argument("--include", nargs="*", default=[], help="Patrones glob para *incluir* (relativos al proyecto)")
    p.add_argument("--exclude", nargs="*", default=[], help="Patrones glob adicionales para *excluir*")
    return p
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    files = gather_files(
        ...
        include_globs=list(args.include) if hasattr(args, 'include') else [],
        exclude_globs=list(args.exclude) if hasattr(args, 'exclude') else [],
    )
    if not files:
        print("No se encontraron archivos .py a analizar (revisa include/exclude).")
        return 1
    if not files:
